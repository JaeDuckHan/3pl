...
router.get("/", async (_req, res) => {
  try {
    const [rows] = await getPool().query(
      `SELECT id, client_id, sku_code, barcode_raw, barcode_full, name_kr, name_en, volume_ml, unit, status, created_at, updated_at
       FROM products
  ...
  try {
    const [result] = await getPool().query(
      `INSERT INTO products (client_id, sku_code, barcode_raw, barcode_full, name_kr, name_en, volume_ml, unit, status)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [client_id, sku_code, barcode_raw, barcode_full, name_kr, name_en, volume_ml, unit, status]
    );
...
router.put("/:id", validate(productUpdateSchema), async (req, res) => {
  ...
  try {
    const [result] = await getPool().query(
      `UPDATE products
       SET client_id = ?, sku_code = ?, barcode_raw = ?, barcode_full = ?, name_kr = ?, name_en = ?, volume_ml = ?, unit = ?, status = ?
       WHERE id = ? AND deleted_at IS NULL`,
      [client_id, sku_code || null, barcode_raw, barcode_full, name_kr, name_en || null, volume_ml || null, unit || null, status, req.params.id]
    );
